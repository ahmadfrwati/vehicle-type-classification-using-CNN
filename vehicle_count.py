...
classNames = class_names = ['bus' , 'car' , 'truck']
#print(classNames)
...
net = cv2.dnn.readNetFromTensorflow('simple_frozen_graph.pb')
#net = cv2.dnn_ClassificationModel('simple_frozen_graph.pb', 'simple_frozen_graph.pbtxt')
...
def realTime():
    ...
    with open("data.csv", 'w') as f1:
        ...
        cwriter.writerow(['bus', 'car', 'truck'])
        up_list.insert(0, "Up")
...
def from_static_image(image):
    ...
    cv2.putText(img, "Truck:      "+str(frequency['truck']), (20, 120), cv2.FONT_HERSHEY_SIMPLEX, font_size, font_color, font_thickness)
    ...
    with open("static-data.csv", 'a') as f1:
        cwriter = csv.writer(f1)
        cwriter.writerow([image, frequency['car'], frequency['bus'], frequency['truck']])
    f1.close()
